...
export default function JobApplications({ jobs, onQuickStatusUpdate }: JobApplicationsProps) {
    ...
    const updateStatus = async (jobId: string, status: JobStatus) => {
        ...
        setUpdating(jobId);
        try {
            await onQuickStatusUpdate(jobId, status);
        } catch (error) {
    ...
    return (
        <div className={`${getCardClasses()} hover:border-green-500/40 transition-all duration-200`}>
            ...
            <div className="border-t border-gray-600 px-4 py-5 sm:p-0">
                {jobs.length === 0 ? renderEmptyState() : (
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="ml-4 flex-shrink-0 relative">
                                            <select
                                                ...
                                                `}
                                                style={{
                                                    backgroundImage: isUpdating ? 'none' : `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%23374151' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`,
